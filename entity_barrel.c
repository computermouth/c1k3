...
void entity_barrel_init(entity_t * e, uint8_t p1, uint8_t p2) {
        ...
        game_entities_enemies.length++;
        game_entities_enemies.entities = realloc(game_entities_enemies.entities, sizeof(entity_t *) * game_entities_enemies.length);
        game_entities_enemies.entities[game_entities_enemies.length - 1] = e;
}
...
void entity_barrel_kill(entity_t * e) {
		...
		// todo, stupid???
		// what happens if there's only one entity, and it dies
		for(uint32_t i = 0; i < game_entities_enemies.length; i++){
		    if (game_entities_enemies.entities[i] == e){
		        game_entities_enemies.length--;
		        // check if it's empty
		        if (game_entities_enemies.length == 0){
		            free(game_entities_enemies.entities);
		            game_entities_enemies.entities = NULL;
		        } else { // otherwise remove
		            entity_t * end = game_entities_enemies.entities[game_entities_enemies.length];
		            game_entities_enemies.entities[i] = end;
		            game_entities_enemies.entities = realloc(game_entities_enemies.entities, sizeof(entity_t *) * game_entities_enemies.length);
		        }
		    }
		}
